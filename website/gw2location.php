...
//	echo ("3");
	header("Access-Control-Allow-Origin: http://bingweb.binghamton.edu");
	...
	if($data['get'] === 'playerdata'){
		...
		foreach($result as $r){
			...
			if(preg_match("#^[A-F\d]{8}-(:?[A-F\d]{4}-){3}[A-F\d]{12}$#i", $r[3])){
				...
				if(!$guild || empty($guild) || !is_array($guild)){
					...
					if(is_array($g) && !in_array('error', $g)){
						$guild = array('name' => $g['guild_name'], 'tag' => $g['tag']);
			...
			$data[$r[0]] = array(
				...
				'map_id' => $r[8],
				'pos' => recalc_coords(json_decode($r[13], 1), json_decode($r[14], 1), array($r[9], $r[10])),
				'angle' => $r[11] * -1,
